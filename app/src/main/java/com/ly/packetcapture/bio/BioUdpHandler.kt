...
import com.ly.packetcapture.bio.BioUdpHandler
import com.ly.packetcapture.db.DatabaseUtil.insert
import com.ly.packetcapture.db.NetData
import com.ly.packetcapture.protocol.dns.DnsPacket
import com.ly.packetcapture.protocol.tcpip.IpUtil
import com.ly.packetcapture.protocol.tcpip.Packet
 ...
 */
class BioUdpHandler(
    var queue: BlockingQueue<Packet>,
            ...
            if (packet.isUDP() && packet.udpHeader.sourcePort == 53) {
                ...
                for (index in 0 until length) {
                    if (dnsPacket.Resources[index].Type.toInt() == 1) {
                        val type = dnsPacket.Resources[index].Type
                        ...
                        stringBuilder.append("域名:" + dnsPacket.Resources[index].Domain + "::::IP地址::::")
                        for (ip in ipList) {
                            val ipInt = ip.trim { it <= ' ' }.toInt()
                            if (ipInt >= 0) {
                                stringBuilder.append(ipInt)
                                ipSb.append(ipInt)
                            } else {
                                stringBuilder.append(ipInt + 256)
                        ...
                        insert(
                            NetData(
                                dnsPacket.Resources[index].Domain,
                                ipSb.substring(0, ipSb.length - 1)
                            )
                        )
                    }
                }
